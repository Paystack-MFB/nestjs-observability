...
import { AlwaysOffSampler, AlwaysOnSampler, BatchSpanProcessor, TraceIdRatioBasedSampler, } from '@opentelemetry/sdk-trace-node';
...
let TracingService = class TracingService {
    ...
    async onApplicationShutdown() {
        if (this.sdk && this.config.tracing.enabled && isTracingSdkInitialized) {
            try {
                ...
                this.logger.error(`Error shutting down tracing: ${shutdownError.message}`, shutdownError.stack ?? '', 'TracingService');
            }
        }
    }
    async onModuleInit() {
        ...
        try {
            ...
            const tracingError = error;
            this.logger.error(`Failed to initialize tracing: ${tracingError.message}`, tracingError.stack ?? '', 'TracingService');
        }
    }
    /**
    ...
    async initializeTracingSdk() {
        ...
        if (isTracingSdkInitialized) {
            ...
            this.logger.warn('Tracing SDK was marked as initialized but no instance found. Reinitializing.', 'TracingService');
        }
        ...
        try {
            ...
            this.logger.warn(`Error starting tracing SDK (may be already started): ${error.message}`, 'TracingService');
        }
    }
};
